...
bool LGGContactSets::hasFriendColorThatShouldShow(const LLUUID& friend_id, ContactSetType type, LLColor4& color) const
{
    ...
    static LLCachedControl<bool> fsContactSetsColorizeFriendsList(gSavedSettings,"FSContactSetsColorizeFriendsList", false);
    ...
    switch (type)
    {
        ...
        case ContactSetType::MINIMAP:
            if (!fsContactSetsColorizeMiniMap)
                return false;
            break;
        case ContactSetType::FRIENDSLIST:
            if (!fsContactSetsColorizeFriendsList)
                return false;
            break;
    };
